...
| ONNX (torch-mlir) | `cd ../torch && make mnist8_onnx.riscv ENTRY=CNTKGraph NARGS=1` (public ONNX model-zoo MNIST classifier) → `forward vs onnxruntime ... ok`, `onnx PASS`; needs onnx/onnxruntime in `/tmp/tmenv` | on Spike above |
