...
All of the above match PyTorch to the harness tolerance on Spike. (GroupNorm is decomposed by torch-mlir
into an f64 grouped reduction; the chansum library kernel is f32-only, so those f64 reductions go through
the generic path, which handles them correctly.)
