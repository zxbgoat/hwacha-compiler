...
private:
  ...
  void releaseAt(unsigned Pos) { for (auto &KV : LastUse) if (KV.second == Pos) release(KV.first); }
  ...
  void emit(StringRef Pred, StringRef Op, ArrayRef<std::string> Ops) {
    ...
    Out << "    " << (Pred.empty() ? "" : ("@" + Pred + " ").str()) << Op;
    for (size_t i = 0; i < Ops.size(); i++) Out << (i ? ", " : " ") << Ops[i];
    Out << "\n";
  }
...
Reg WTGen::alloc(RC C) {
  ...
  unsigned Start = (C == RC::VV || C == RC::VW) ? 0 : 1, Limit = (C == RC::VV || C == RC::VW) ? 256 : C == RC::VS ? 64 : 16;
  if (Used.size() < Limit) Used.resize(Limit, false);
  for (unsigned i = Start; i < Limit; i++)
    if (!Used[i]) {
      Used[i] = true;
      if (C == RC::VV) NumVV = std::max(NumVV, i + 1);
      if (C == RC::VP) NumVP = std::max(NumVP, i + 1);
      if (C == RC::VS) NumVS = std::max(NumVS, i + 1);
...
void WTGen::computePositions() {
  unsigned Pos = 0;
  DenseMap<const Value *, unsigned> DefPos;
  std::vector<std::pair<Loop *, unsigned>> Open;
  auto use = [&](Value *V, unsigned P) { LastUse[V] = std::max(LastUse[V], P); };
  for (Item &It : Order) {
    Pos++;
...
void WTGen::movePred(unsigned Pred, Reg Dst, Value *Src) {
  ...
  else emit(P, Dst.Class == RC::VW ? "vaddw" : "vadd", {Dst.str(), S, "vs0"});
}
...
bool WTGen::endLoop(Loop *L, unsigned &Pos) {
  ...
  for (PHINode &Phi : H->phis()) movePred(Back, RegOf[&Phi], Phi.getIncomingValueForBlock(Latch));
  ...
  for (auto &KV : LS.ExitTotal) {
    // record as a synthetic edge (loop -> exit block); consumed when the exit block is emitted
...
bool WTGen::emitInst(Instruction &I, unsigned Pos) {
  ...
  if (auto *Cast = dyn_cast<CastInst>(&I)) {
    ...
    case Instruction::ZExt: case Instruction::SExt: {
      ...
      if (Cast->getOpcode() == Instruction::SExt) {   // sext of wider ints is a no-op (registers hold sign-extended values)
        RegOf[&I] = regOfValue(Src); Alias.insert(&I);
        if (auto *SI = dyn_cast<Instruction>(Src)) LastUse[SI] = std::max(LastUse[SI], LastUse[&I]);
        return true;
      }
...
static Value *cloneUniform(Value *V, ValueToValueMapTy &VMap, IRBuilder<> &B, raw_ostream &Err) {
  ...
  if (auto *GV = dyn_cast<GlobalVariable>(V)) {
    ...
    if (!C) {
      C = new GlobalVariable(CT, GV->getValueType(), false, GlobalValue::InternalLinkage,
