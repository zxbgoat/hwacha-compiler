...
static bool emit(ModuleOp m, StringRef path, StringRef name) {
  ...
  std::error_code ec;
  // Optionally strip large constant globals (model weights) out of the IR into a binary blob referenced
  // by a .incbin .S stub, so the huge dense arrays never become .word text: keeps this .ll and the
  // downstream hwacha-cc .s small, and their parse memory bounded, for big models (DiT-L/XL, etc.).
  if (!WeightsBin.empty() && name == "hwacha-host") {
    std::string sPath = WeightsBin + ".S";
    std::error_code ecb, ecs;
    llvm::raw_fd_ostream bin(WeightsBin, ecb, llvm::sys::fs::OF_None);
    if (ecb) { llvm::errs() << "hwacha-mlir: " << WeightsBin << ": " << ecb.message() << "\n"; return false; }
    llvm::raw_fd_ostream asmf(sPath, ecs, llvm::sys::fs::OF_Text);
    if (ecs) { llvm::errs() << "hwacha-mlir: " << sPath << ": " << ecs.message() << "\n"; return false; }
    std::string binName = std::string(llvm::sys::path::filename(WeightsBin));
    asmf << "// generated by hwacha-mlir: constant globals stripped from " << name
         << " into " << binName << " (referenced by .incbin)\n.section .rodata\n";
    uint64_t off = 0, n = 0, bytesTotal = 0;
    ...
    for (llvm::GlobalVariable &G : lm->globals()) {
      ...
      uint64_t sz = lm->getDataLayout().getTypeAllocSize(init->getType());
      ...
      int64_t wrote = streamConstBytes(init, lm->getDataLayout(), bin);
      if (wrote < 0) { llvm::errs() << "hwacha-mlir: cannot serialize " << G.getName() << ", leaving inline\n"; continue; }
      asmf << ".balign " << align << "\n.globl " << G.getName() << "\n" << G.getName() << ":\n"
           << ".incbin \"" << binName << "\", " << off << ", " << wrote << "\n";
      off += wrote; bytesTotal += wrote; n++;
      G.setInitializer(nullptr);
      G.setLinkage(llvm::GlobalValue::ExternalLinkage);
    }
    if (PrintMLIR) llvm::errs() << "// ---- weights: stripped " << n << " globals (" << bytesTotal
                                << " bytes) to " << WeightsBin << " + " << sPath << "\n";
  }
